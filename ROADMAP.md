...
## 🎯 Now
Current: **#15 Go server rewrite** — for a *very fast* server the JS/Node server is being rewritten in **Golang** (Go 1.26), same wire protocol + UDP beacon + **event-driven pipe-pane** (NO polling — user rule). Porting pure libs first (beacon ✅, input FSM ✅), then ansi/detect/format/discovery, then the tmux backend + WS + beacon wiring. `server/` (JS) stays as the reference until `server-go/` reaches parity, then is retired.
...
**📅 Day 2 — 2026-08-15/16 · Go rewrite (very fast, event-driven)**
15. 🟡 **Rewrite the server in Golang.** *(current)* Motivation: a *very fast* WS server; house rule *no polling / no defensive loops* — updates are event-driven off tmux `pipe-pane` (fifo change-signal → capture+push; session death detected on stream close; no timers). `server-go/` (module `cardputerme`, Go 1.26). Ported + tested: `beacon.go`, `input.go` (FSM). TODO: `ansi.go` (no-regex ANSI→RGB565), `detect.go` (parseChoices/endsWithQuestion), `format.go` (toAscii/wrapLine/sliceIntoCards), `discovery.go` (pickPort 8001–8255), `viewport.go`, `display.go`, then `terminal.go` (tmux backend + `subscribe`), `main.go` (HTTP `/health`, WS `/ws`, UDP beacon). WS lib: `gorilla/websocket` (needs `go get`). Keep the JS `server/` as reference/oracle until parity; the JS event-driven version (last commit) is the design spec.

...
## 🎯 Now
Current: **#8 device E2E — CORE PASSING on hardware (2026-08-15)**: firmware flashed; Cardputer (192.168.0.69) discovered servers via UDP beacon, connected over WS, mirrored + ran commands + drove the live Claude Code session, and answered an on-device question prompt. Now on the restructured Go binary (`cmd/`+`internal/`). **3 real bugs found+fixed on-device:** (1) beacon needed subnet-directed broadcast (`652d4f7`); (2) prompt detection now reads the blank-trimmed grid the device shows, not raw last-N rows (menus with trailing blanks were missed); (3) `lastAwaiting` syncs on client connect so a digit answers a prompt already on screen. Remaining to eyeball: history recall, autosuggest ghost, fn+esc back, marquee. Test harness live: `ask` exposure runs an interactive `1/2/3` menu. **tmux backend stays (user) — PTY parked.**
> **#15 Go rewrite is DONE** — `server-go/` is at parity (wire smoke: beacon→connect→type→mirror; event-driven self-exit on terminal death, zero polling), the launcher builds+execs the Go binary, and the JS `server/` is retired (−658 files). `go test ./...` green (53 tests).
...
## Done 2026-08-15 (Go CLI restructure — #16)
- **#16 Conventional Go CLI layout.** `server-go/` → `cmd/cardputerme/main.go` (thin entry) + `internal/{screen,input,discovery,terminal,server}` cohesive packages with exported APIs; `Server` struct replaces package globals (idiomatic, testable). JS `server/` fully removed from disk. Dead code removed (`EndsWithQuestion`), verified by `deadcode` + `go vet`. 63 tests green across packages. Firmware flashed + E2E core passing on hardware (device 192.168.0.69 drove the live Claude Code session). Beacon subnet-broadcast fix (`652d4f7`) + WS connect logging.

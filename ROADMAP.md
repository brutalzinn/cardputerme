...
## 🎯 Now
Current: **#8 device E2E — CORE PASSING on hardware (2026-08-15)**: firmware flashed; Cardputer (192.168.0.69) discovered servers via UDP beacon, connected over WS, mirrored + ran commands, and is now mirroring the live Claude Code session (`cardputerme`, port 8002). Bug found+fixed on-device: beacon needed subnet-directed broadcast (`652d4f7`). Remaining checklist to eyeball: choice menu, history recall, autosuggest ghost, fn+esc back, marquee. **tmux backend stays (user, 2026-08-15) — it works; PTY backend remains parked, not now.**
